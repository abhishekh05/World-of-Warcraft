...
###############################################################################################################
## PERSONAL TODO:
## 	- Insert level names for categorical variables (i.e. quality, class, etc.)
## 	 	> I'm working on a reference document to send to you all later today.
## 	- Troubleshoot k-means clustering analysis
## 	- Troubleshoot locale issue with year/month/day parser
##  - Clean up plots (i.e. add in better titles and axis titles)
## 	- Combine plots where appropriate
##
## THINGS TO NOTE:
## 	- For prototyping purposes, I randomly split the data into a smaller size to reduce computational time
## 	  In the end I will run this again with the full set of ovbservations once we're satisfied with the plots
##  - I specifically only focused on item data from the auction house due to reduce our work load
## 	- Let me know if there are somethings I missed/you want to see or if there's something erroneous
## 	- When it comes to cleaning up the dataset, it might be worth looking at the boxplots, and time
## 	  series plots. When we meet up again, we can also disscuss our interpretations of the EDA
## 	- There's something tosay about items wD:\SDM projectith coded with quality 0
################################################################################################################     
...
itemPrices <- read_csv("daily_prices.csv") #14,888,271 rows 11 columns
itemMapper<- read_csv("itemID_name_mapper.csv") # 111,165 rows, 27 columns
...
la
sso.pred2 <- predict(lasso.mod, s = bestlam, newx = X[-train,], type = "response")
